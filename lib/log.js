...
//
// Default: OFF. Users see no log spam. To enable for local debugging:
//   chrome.storage.local.set({ adnotaDebugLog: true })    // global, all surfaces
//   localStorage.setItem('adnotaDebugLog', '1')           // per-tab override
// To turn back off:
...
(function () {
  ...
  // Default off so a fresh install is silent. Page-side localStorage opt-IN
  // ('1') flips this on for a single tab; chrome.storage.local opt-IN flips
  // it on globally across every Adnota surface.
  const state = { enabled: false, localForce: false };

  if (!isWorker) {
    try { state.localForce = localStorage.getItem(STORAGE_KEY) === '1'; } catch {}
  }

  // chrome.storage.local read + live-toggle listener. Available everywhere
  // (content scripts, service worker, popup, sites page). No first-run seed
  // — the key just stays absent until a developer flips it.
  ...
  function isOn() { return state.enabled || state.localForce; }
