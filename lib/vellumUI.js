...
window.VellumUI = {
  ...
  // ─── Page-annotation count (read-only sibling of clearPageAnnotations) ──────
  // Uses identical matching rules so the number shown in the confirm dialog
  // exactly equals the number that will be deleted on confirm.

  async countPageAnnotations(actionTypes) {
    const domain = location.hostname;
    const path = location.pathname;
    const actionSet = new Set(actionTypes);

    const data = await chrome.storage.local.get(domain);
    const items = data[domain]?.items;
    if (!items) return 0;
    ...
    for (const item of items) {
      const onThisPage = item.path === path || item.path === '*';
      const itemAction = item.action || 'ERASE';
      if (onThisPage && actionSet.has(itemAction)) n++;
    }
    return n;
  },
  ...
  // Shared helper used by eraser/sticky/drawing HUDs. On click, counts the
  // items that would be removed, shows a branded confirm dialog with the
  // live count, then clears + reloads. No-ops silently when count is 0.
  //   singular/plural — used to pluralize both tooltip and dialog text.
  ...
  createTrashButton({ singular, plural, actionTypes, title }) {
    ...
    return window.VellumUI.createToolbarIconButton(
      ...
      hoverText,
      async () => {
        const n = await window.VellumUI.countPageAnnotations(actionTypes);
        if (n === 0) {
