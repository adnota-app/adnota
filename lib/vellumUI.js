...
window.VellumUI = {
  ...
  // viewport-dominating containers, or as soon as a parent meaningfully grows.
  bubbleToVisualRoot(el, {
    maxHops = 8,
    edgeTolerancePx = 4,
    sizeToleranceRatio = 0.05,
  } = {}) {
    let current = el;
    ...
    for (let i = 0; i < maxHops; i++) {
      ...
      if (Math.abs(pRect.top - rect.top) > edgeTolerancePx) break;
      if (Math.abs(pRect.left - rect.left) > edgeTolerancePx) break;
      if (Math.abs(pRect.right - rect.right) > edgeTolerancePx) break;
      if (Math.abs(pRect.bottom - rect.bottom) > edgeTolerancePx) break;
