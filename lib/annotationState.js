...
window.AdnotaState = {
  ...
  set(patch) {
    ...
    if (patch.color) {
      chrome.storage.local.set({ adnotaHighlightColor: patch.color });
    }
    if (patch.strokeWidth) {
      chrome.storage.local.set({ adnotaStrokeWidth: patch.strokeWidth });
    }
    if ('filled' in patch) {
      chrome.storage.local.set({ adnotaShapeFilled: !!patch.filled });
    }
    if ('mode' in patch) {
      ...
      chrome.storage.local.set({ adnotaActiveMode: this.mode });
    }
