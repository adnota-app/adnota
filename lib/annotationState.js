...
window.AdnotaUndo = {
  ...
  // Drop every pending entry. Called from the restorer's SPA-teardown so
  // undo history doesn't survive a route change — each entry's closure
  // captures DOM refs and storage state from the previous route, and
  // executing one after the route has been torn down at best no-ops and
  // at worst mutates storage for a page the user isn't on anymore.
  // A full page reload already wipes history (the content-script context
  // is destroyed); SPA nav reuses the context, so the symmetry has to be
  // explicit.
  clear() {
    if (this._stack.length === 0) return;
    window.AdnotaLog?.event('state', 'undo-clear', { stackDepth: this._stack.length });
...
// Re-entrance guard. On sites with their own focus management (GitHub, some
// SPA frameworks), our .focus() synchronously triggers a focusin which the
// page's handler reacts to by re-focusing one of its own elements. That
// fires *our* focusin listener (line below), which calls back into here.
// Without the guard, the page and Adnota fight in a synchronous loop until
// the stack overflows ("Maximum call stack size exceeded"). With the guard:
// we attempt re-anchor once per user interaction, the page wins focus that
// round if it's aggressive about it, and the user's next event starts a
// fresh attempt. Degraded focus-anchoring on those sites, but no crash.
