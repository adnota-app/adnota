...
window.VellumState = {
  ...
  mode: null,
  // Either a theme class ('vellum-theme-yellow') or a raw hex ('#ff0033') picked
  ...
  set(patch) {
    ...
    if ('filled' in patch) {
      chrome.storage.local.set({ vellumShapeFilled: !!patch.filled });
    }
    if ('mode' in patch) {
...
// Escape unreachable. We park focus on a hidden Vellum-owned element whenever
// any tool is active, and yank it back if anything else grabs it.
// ---------------------------------------------------------------------------
const vellumFocusAnchor = document.createElement('div');
vellumFocusAnchor.id = 'vellum-focus-anchor';
vellumFocusAnchor.setAttribute('data-vellum-ui', '1');
vellumFocusAnchor.setAttribute('tabindex', '-1');
vellumFocusAnchor.setAttribute('aria-hidden', 'true');
vellumFocusAnchor.style.cssText =
  'position:fixed;top:0;left:0;width:0;height:0;opacity:0;pointer-events:none;outline:none;';

function anchorVellumFocus() {
  if (!window.VellumState.mode) return;
  if (!vellumFocusAnchor.isConnected) {
    (document.body || document.documentElement).appendChild(vellumFocusAnchor);
  }
  try { vellumFocusAnchor.focus({ preventScroll: true }); } catch {}
}
...
window.VellumState.anchorFocus = anchorVellumFocus;

window.VellumState.subscribe((state) => {
  if (state.mode) anchorVellumFocus();
});

// Any focus move outside Vellum UI while a tool is active → steal it back.
window.addEventListener('focusin', (e) => {
  if (!window.VellumState.mode) return;
  if (!e.target || e.target === vellumFocusAnchor) return;
  if (e.target.closest && e.target.closest('[data-vellum-ui]')) return;
  anchorVellumFocus();
}, true);
...
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'visible') anchorVellumFocus();
});
...
window.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && window.VellumState.mode) {
    window.VellumState.set({ mode: null });
  }
}, true);

// Restore persisted color preference on load.
chrome.storage.local.get(['vellumHighlightColor', 'vellumStrokeWidth', 'vellumShapeFilled'], (result) => {
  ...
  if (typeof result.vellumShapeFilled === 'boolean') {
    window.VellumState.set({ filled: result.vellumShapeFilled });
  }
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'vellum-soft-delete') {
    window.VellumUI?.softDeleteItems({
