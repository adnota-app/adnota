...
(function () {
  ...
  // Which size-cap axes, if any, come from Adnota's own resize rules rather
  // than the page's CSS. A `max-width` / `max-height` the resizer injected
  // isn't a page constraint — it's just the size the user dragged to (the
  // height-shrink drag path persists `max-height: <h>`, and finite-scroll
  // persists `max-height` too). Reading those back as "you've hit your max"
  // is self-referential noise, so detectSizeCaps drops the axes we own.
  // `window.AdnotaResizeRules` is the source of truth for the resizer's
  // `<style id="adnota-style-overrides">` tag; an empty/absent Map (tool
  // never used) just yields {w:false, h:false}.
  function ownResizeCapAxes(el) {
    const rules = window.AdnotaResizeRules;
    if (!rules || !el || typeof el.matches !== 'function') return { w: false, h: false };
    let w = false, h = false;
    for (const [, rule] of rules) {
      if (w && h) break;
      let matchesSel = false;
      try { matchesSel = el.matches(rule.selector); } catch (_) { continue; }
      if (!matchesSel) continue;
      if (rule.cssText.includes('max-width')) w = true;
      if (rule.cssText.includes('max-height')) h = true;
    }
    return { w, h };
  }
  ...
  function detectSizeCaps(el) {
    ...
    const own = ownResizeCapAxes(el);
    const maxW = own.w ? null : readPxCap(cs.maxWidth);
    const maxH = own.h ? null : readPxCap(cs.maxHeight);
