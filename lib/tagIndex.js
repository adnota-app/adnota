...
(function () {
  ...
  function buildAutocompleteDropdown(inputEl, opts = {}) {
    ...
    function ensureDropdown() {
      ...
      dropdown.className = 'adnota-tag-suggest';
      dropdown.setAttribute('data-adnota-ui', '1');
    ...
    function render() {
      ...
      matches.forEach(({ tag, count }, i) => {
        ...
        item.className = 'adnota-tag-suggest-item' + (i === highlightedIdx ? ' highlighted' : '');
        item.dataset.tag = tag;

        const hash = document.createElement('span');
        hash.className = 'adnota-tag-suggest-item-hash';
        hash.textContent = '#';
        item.appendChild(hash);

        const name = document.createElement('span');
        name.className = 'adnota-tag-suggest-item-name';
        name.textContent = tag;
        item.appendChild(name);

        const countEl = document.createElement('span');
        countEl.className = 'adnota-tag-suggest-item-count';
    ...
    function onInput() {
      // Auto-highlight the first match while the user is narrowing the list,
      // so Tab/Enter pick the top suggestion without an arrow-down first.
      // Empty query stays unhighlighted (no implicit preference yet).
      const q = normalize(inputEl.value).toLowerCase();
      highlightedIdx = q ? 0 : -1;
      render();
    }
    ...
    function onKeyDown(e) {
      ...
      const items = dropdown.querySelectorAll('.adnota-tag-suggest-item');
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        ...
        render();
      } else if (e.key === 'Tab' && !e.shiftKey && highlightedIdx >= 0 && items[highlightedIdx]) {
        // Pick the highlighted suggestion without surrendering focus.
        // Shift+Tab is left alone so users can always escape backward.
        e.preventDefault();
        ...
        if (highlightedIdx >= 0 && items[highlightedIdx]) {
          ...
          // No suggestion to pick — close the dropdown so Enter feels
          // responsive. The typed value is already in the input.
          hide();
        }
  ...
  window.AdnotaTags = { getAllTags, normalize, buildAutocompleteDropdown };
})();
