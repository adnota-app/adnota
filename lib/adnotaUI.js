...
window.AdnotaUI = {
  ...
  async softDeleteItems({ singular, plural, actionTypes, skipConfirm, scope }) {
    ...
    //   'site' — items where path === '*' (strict site-wide; excludes
    //            curated url-scope rules — a "Clear Site-Wide" click
    //            shouldn't nuke a scope:google-web rule)
    //   default — the "applies to this page" union; delegates to
    //            AdnotaStorage.matchesUrl so curated url-scope rules are
    //            included on the page they apply to.
    const scopeFilter = (item) => {
      if (scope === 'page') return item.path === path;
      if (scope === 'site') return item.path === '*';
      return window.AdnotaStorage.matchesUrl(item, location.href);
    };
