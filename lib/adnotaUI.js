...
window.AdnotaUI = {
  ...
  async softDeleteItems({ singular, plural, actionTypes, skipConfirm }) {
    ...
    if (snapshot.length === 0) {
      window.AdnotaLog?.event('trash-all', 'noop', { actionTypes });
    ...
    window.AdnotaLog?.event('trash-all', 'invoke', { actionTypes, count: n });
  ...
  // ─── Flash a brief purple pulse over a set of viewport rects ──────────────
  // Used by Highlighter.scrollTo and StickyEngine.scrollTo to mark where the
  // user just landed, since smooth scroll on a long page can feel
  // disorienting otherwise. Rects are viewport-relative (getBoundingClientRect
  // / range.getClientRects shape); the layer is position:fixed so they stay
  // accurate. Animation lives in lib/adnotaUI.css under .adnota-flash-rect.
  flashRects(rects, opts) {
    if (!rects || !rects.length) return;
    const durationMs = opts?.durationMs ?? 800;
    const layer = document.createElement('div');
    layer.setAttribute('data-adnota-ui', '1');
    layer.style.cssText =
      'position:fixed;left:0;top:0;width:0;height:0;pointer-events:none;z-index:2147483647;';
    for (const r of rects) {
      if (!r || r.width <= 0 || r.height <= 0) continue;
      const div = document.createElement('div');
      div.className = 'adnota-flash-rect';
      div.style.cssText =
    ...
    setTimeout(() => layer.remove(), durationMs + 50);
  },
};
