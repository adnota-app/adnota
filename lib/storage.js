...
const AdnotaStorage = {
  // Scope key to use when SAVING an item committed from `url`. Returns the
  // curated scope (e.g. `scope:google-web`) for domains in urlScopeRegistry,
  // or `'*'` (site-wide) otherwise. Centralized so resizer save sites don't
  // each duplicate the `resolveScope() ?? '*'` fallback.
  pathForSave(url) {
    try {
      return window.AdnotaUrlScopeRegistry?.resolveScope(url) ?? '*';
    } catch {
      return '*';
    }
  },

  // True if a stored `item` should apply when the page is at `url`. Single
  // source of truth used by both the per-page loader and the resizer's
  // reset path so curated-scope items are matched and cleaned consistently
  // with how they're loaded.
  matchesUrl(item, url) {
    try {
      if (item.path === '*') return true;
      const parsed = new URL(url);
      if (item.path === parsed.pathname) return true;
      const scope = window.AdnotaUrlScopeRegistry?.resolveScope(url);
      return !!scope && item.path === scope;
    } catch {
      return false;
    }
  },
  ...
  // Bulk append for batch commits. Critical: N parallel saveItem calls
  // would each read the same "before" state and overwrite each other on
  // write (last-write-wins). One read-modify-write that appends all
  // entries at once side-steps the race entirely. entries: [{ path, payload }].
  async saveItems(domain, entries) {
  ...
  async getAnchorsForUrl(url) {
    try {
      ...
      return domainData.items.filter(item => this.matchesUrl(item, url));
    } catch (err) {
      return [];
    }
  },
